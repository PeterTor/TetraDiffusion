...
######################################################################################
@torch.no_grad()
def valid_path(mtl_path, texture_path):
    texture_root = os.path.dirname(texture_path)
    if os.path.exists(os.path.join(mtl_path, texture_root)):
        path = os.path.join(mtl_path, texture_path)
    elif os.path.exists(os.path.join(mtl_path, "." + texture_root)):
        path = os.path.join(mtl_path, "." + texture_path)
    else:
        raise FileNotFoundError(f"Cannot find {os.path.join(mtl_path, texture_path)}")
    return path
...
def load_mtl(fn, clear_ks=True):
    ...
    for line in lines:
        ...
        if 'newmtl' in prefix:
            ...
            if 'bsdf' in prefix or 'map_kd' in prefix or 'map_ks' in prefix or 'bump' in prefix or 'map_d' in prefix:
    ...
    for mat in materials:
        ...
        if 'map_kd' in mat:
            path = valid_path(mtl_path, mat['map_kd'])
            mat['kd'] = texture.load_texture2D(path)
        else:
            mat['kd'] = texture.Texture2D(mat['kd'])
        
        if 'map_ks' in mat:
            path = valid_path(mtl_path, mat['map_ks'])
            mat['ks'] = texture.load_texture2D(path, channels=3)
        else:
            mat['ks'] = texture.Texture2D(mat['ks'])

        if 'bump' in mat:
            path = valid_path(mtl_path, mat['bump'])
            mat['normal'] = texture.load_texture2D(path, lambda_fn=lambda x: x * 2 - 1, channels=3)
...
def merge_materials(materials, texcoords, tfaces, mfaces):
    ...
    if full_res[1]>100000:
        full_res[1] = full_res[1]/2
    if full_res[1] > 100000:
        full_res[1] = 50000
    print(full_res)
    ...
    for tex in textures:
        if tex in materials[0]:
            ...
            tex_data = torch.cat(tuple(util.scale_img_nhwc(mat[tex].data, tuple(max_res))[..., :3] for mat in materials), dim=2) # Lay out all textures horizontally, NHWC so dim2 is x
    ...
    for fi in range(len(tfaces)):
        ...
        for vi in range(3):
            ...
            if ti == -1:
                ...
                texcoords_ti_0 = texcoords[ti][0]
                texcoords_ti_1 = texcoords[ti][1]
            if not (ti in new_tverts):
                new_tverts[ti] = {}
            if not (matIdx in new_tverts[ti]): # create new vertex
                new_tverts_data.append([(matIdx + texcoords_ti_0) / s_coeff[1], texcoords_ti_1 / s_coeff[0]]) # Offset texture coodrinate (x direction) by material id & scale to local space. Note, texcoords are (u,v) but texture is stored (w,h) so the indexes swap here

...
def _load_env_hdr(fn, scale=1.0):
    print("Loading hdr")
    latlong_img = torch.tensor(util.load_image(fn, hdr=True), dtype=torch.float32, device='cuda')*scale
    print("Now cube mapping")
    cubemap = util.latlong_to_cubemap(latlong_img, [512, 512])
    print("Creating env and building mips")

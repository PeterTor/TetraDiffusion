...
def rgb_to_srgb(f: torch.Tensor) -> torch.Tensor:
    ...
    color[..., 2] -= 128.
    color = lab2rgb(color)
    color = torch.from_numpy(color).to(f.device).to(torch.float32)
    return color
...
def srgb_to_rgb(f: torch.Tensor) -> torch.Tensor:
    return f
    color = f.cpu().numpy()
    color = rgb2lab(img_as_int(color))
    color[..., 0] /= 100.
    color[..., 1] += 128.
    color[..., 1] /= 255.
    color[..., 2] += 128.
...
from pytorch3d.transforms import random_rotation as r_rot
@torch.no_grad()
def random_rotation_translation(t, device=None):
    ...
    #m[:3, :3] = r_rot(dtype=torch.float32, device=device)
    # m[:3, 3] = (-t - t) * torch.rand([3]) + t
    #m[3, 3] = 1.0
    #return m
...
def load_image_raw(fn) -> np.ndarray:
    ...
    if os.path.exists(base + ext):
        return imageio.v3.imread(fn)
    elif os.path.exists(base + ext.lower()):
        return imageio.v3.imread(base + ext.lower())
    elif os.path.exists(base + ext.upper()):
        return imageio.v3.imread(base + ext.upper())
    else:
        raise FileNotFoundError(f"Cannot find {fn}")

def load_image(fn, hdr=False) -> np.ndarray:
    img = load_image_raw(fn)
    print("Loaded image, now resizing if necessary")
    print(img.shape)
    if img.shape[0] > 1024 and not hdr:
        scale_percent = 512/img.shape[0]  # percent of original size
    ...
    elif img.shape[1] > 1024 and not hdr:
        scale_percent = 512/img.shape[1]  # percent of original size
        width = int(img.shape[1] * scale_percent)
        height = int(img.shape[0] * scale_percent)
        dim = (width, height)
        ...
        # resize image
        img = cv2.resize(img, dim, interpolation=cv2.INTER_AREA)
    print("Returng img of size ", img.shape)

...
######################################################################################
class Mesh:
    def __init__(self, v_pos=None, t_pos_idx=None, v_nrm=None, t_nrm_idx=None, v_tex=None, t_tex_idx=None, v_tng=None, t_tng_idx=None, material=None, base=None, v_color=None):
        self.v_pos = v_pos
        self.v_nrm = v_nrm
        self.v_tex = v_tex
        self.v_tng = v_tng
    ...
    def copy_none(self, other):
        ...
        if self.v_color is None:
            self.v_color = other.v_color
    ...
    def clone(self):
        ...
        if out.v_color is not None:
            out.v_color = out.v_color.clone().detach()
...
def load_mesh(filename, mtl_override=None, scale=0.95):
    name, ext = os.path.splitext(filename)
    if ext == ".obj":
        print("loading ",filename)
        return obj.load_obj(filename, clear_ks=True, mtl_override=mtl_override, scale=scale)

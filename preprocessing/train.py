...
def prepare_batch(target, bg_type='black'):
    ...
    target['img_second'] = torch.cat((torch.lerp(background, target['img_second'][..., 0:3],target['img_second'][..., 3:4]), target['img_second'][..., 3:4]),dim=-1)
...
def validate_itr(glctx, target, geometry, opt_material, lgt, FLAGS):
    ...
    with torch.no_grad():
        ...
        _, buffers = geometry.render(glctx, target, lgt, opt_material)
        result_dict['ref'] = util.rgb_to_srgb(target['img'][...,0:3])[0]
        result_dict['opt'] = util.rgb_to_srgb(buffers['shaded'][...,0:3])[0]
        result_dict['depth'] = target['depth'][...,0:3][0]
        result_dict['mask'] = target['mask'][...,0:3][0]
        result_dict['normal'] = target['normal'][...,0:3][0]
        result_image = torch.cat([result_dict['opt'], result_dict['ref'], result_dict['normal'], result_dict['depth'], result_dict['mask']], axis=1)
        ...
        if FLAGS.display is not None:
            ...
            for layer in FLAGS.display:
                if 'latlong' in layer and layer['latlong']:
                    ...
                    _, img = geometry.render(glctx, target, layer['relight'], opt_material)
                    result_dict['relight'] = util.rgb_to_srgb(img[..., 0:3])[0]
                    result_image = torch.cat([result_image, result_dict['relight']], axis=1)
                elif 'bsdf' in layer:
                    _, buffers = geometry.render(glctx, target, lgt, opt_material, bsdf=layer['bsdf'])
                    if layer['bsdf'] == 'kd':
                        result_dict[layer['bsdf']] = util.rgb_to_srgb(buffers['shaded'][0, ..., 0:3])  
                    elif layer['bsdf'] == 'normal':
                        result_dict[layer['bsdf']] = (buffers['normal'][0, ..., 0:3] + 1) * 0.5
                    elif layer['bsdf'] == 'depth':
                        result_dict[layer['bsdf']] = buffers['depth'][0, ..., 0:3]
                    elif layer['bsdf'] == 'mask':
                        result_dict[layer['bsdf']] = buffers['mask'][0, ..., 0:3]
                    else:
...
def optimize_mesh(
    ...
    def lr_schedule(iter, fraction):
        ...
        return max(1e-5, 10**(-(iter - warmup_iter)*0.0002)) # Exponential falloff from [1.0, 0.1] over 5k epochs.
    ...
    if FLAGS.multi_gpu: 
        ...
        if optimize_geometry:
            ...
            scheduler_mesh = torch.optim.lr_scheduler.LambdaLR(optimizer_mesh, lr_lambda=lambda x: lr_schedule(x, 0.95))
            # scheduler_mesh = torch.optim.lr_scheduler.StepLR(optimizer_mesh, step_size=250, gamma=1.0)

        optimizer = apex.optimizers.FusedAdam(trainer_noddp.params, lr=learning_rate_mat)
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda x: lr_schedule(x, 0.95))
        # scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=250, gamma=1.0)
    else:
        ...
        if optimize_geometry:
            ...
            scheduler_mesh = torch.optim.lr_scheduler.LambdaLR(optimizer_mesh, lr_lambda=lambda x: lr_schedule(x, 0.95))
            # scheduler_mesh = torch.optim.lr_scheduler.StepLR(optimizer_mesh, step_size=250, gamma=0.1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser.add_argument('-scl', '--scale', type=int)
        ...
        if FLAGS.out_dir is None:
            ...
            #if FLAGS.dmtet_grid == 256:
            #    print("writing 256")
            #    FLAGS.out_dir = 'out_256/' + FLAGS.out_dir
            #elif FLAGS.dmtet_grid == 192:
            #    FLAGS.out_dir = 'out_192/' + FLAGS.out_dir
            #else:
        ...
        if os.path.splitext(FLAGS.ref_mesh)[1] == '.obj':
            ref_mesh         = mesh.load_mesh(FLAGS.ref_mesh, FLAGS.mtl_override, scale=FLAGS.scale_factor)
        ...
        if FLAGS.base_mesh is None:
            ...
            # axis_vector = torch.tensor([[ 0.1940,  2.1614, -0.1721]], device='cuda:0') #torch.randn((1, 3), device='cuda')
            # any_vector = torch.tensor([[ 0.1391, -0.1082, -0.7174]], device='cuda:0') #torch.randn((1, 3), device='cuda')
            # print(torch.randn((1, 3), device='cuda'))
            # print(torch.randn((1, 3), device='cuda'))


            geometry = DMTetGeometry(FLAGS.dmtet_grid, FLAGS, dataset_train[0]['orig_mesh'])

            # Setup textures, make initial guess from reference if possible
            mat = initial_guess_material(geometry, False, FLAGS)

            # Run optimization
            geometry, _ = optimize_mesh(glctx, geometry, mat, lgt, dataset_train, dataset_validate,
            ...
            base_mesh = geometry.getMesh(mat,mask=True).clone()
            # Free temporaries / cached memory
            torch.cuda.empty_cache()

            lgt = lgt.clone()
            os.makedirs(os.path.join(FLAGS.out_dir, "dmtet_data"), exist_ok=True)

            deform = geometry.deform.cpu().detach().numpy()
            color = geometry.color.cpu().detach().numpy()
            sdf = geometry.sdf.cpu().detach().numpy()
            sdf[geometry.mask.cpu().detach().numpy()] = 1.0

            np.savez_compressed(os.path.join(FLAGS.out_dir, "dmtet_data", "deform.npz"), deform)
            np.savez_compressed(os.path.join(FLAGS.out_dir, "dmtet_data", "color.npz"), color)
            np.savez_compressed(os.path.join(FLAGS.out_dir, "dmtet_data", "sdf.npz"), sdf)

            if FLAGS.local_rank == 0:
                mesh = trimesh.Trimesh(vertices=base_mesh.v_pos.detach().cpu().numpy(), faces=base_mesh.t_pos_idx.detach().cpu().numpy())
                colors = util.rgb_to_srgb(torch.clamp(base_mesh.v_color, 0, 1).detach().cpu()[None, ...])[0] * 255
                mesh.visual.vertex_colors = colors.to(torch.uint8).numpy()
                mesh.export(os.path.join(FLAGS.out_dir, "dmtet_data", "mesh.obj"))
            geometry.sdf.data[geometry.sdf.data == 0.0] = 1.0
            geometry.sdf.data = torch.sign(geometry.sdf.data).cuda() #torch.round(geometry.sdf.data, decimals=1)
            geometry.sdf.requires_grad = False
            geometry.deform_scale = 2.0
            geometry.deform.data[:] = geometry.deform * (0.45/geometry.deform_scale)
            geometry.with_laplace = True
            # ==============================================================================================
            ...
            print(torch.unique(geometry.sdf))
        ...
        if FLAGS.local_rank == 0:
            ...
            color = geometry.color.cpu().detach().numpy()
            sdf = geometry.sdf.cpu().detach().numpy()
            os.makedirs(os.path.join(FLAGS.out_dir, "mesh_data"), exist_ok=True)
            np.savez_compressed(os.path.join(FLAGS.out_dir, "mesh_data", "deform.npz"), deform)
            np.savez_compressed(os.path.join(FLAGS.out_dir, "mesh_data", "color.npz"), color)
            np.savez_compressed(os.path.join(FLAGS.out_dir, "mesh_data", "sdf.npz"), sdf)
            torch.save([sdf,deform,color], os.path.join(FLAGS.out_dir, "mesh_data", "sample.pth"))

            # obj.write_obj(os.path.join(FLAGS.out_dir, "mesh/"), final_mesh)
            mesh = trimesh.Trimesh(vertices=final_mesh.v_pos.detach().cpu().numpy(), faces=final_mesh.t_pos_idx.detach().cpu().numpy())
            colors = util.rgb_to_srgb(final_mesh.v_color.detach().cpu()[None, ...])[0] * 255
            mesh.visual.vertex_colors = colors.to(torch.uint8).numpy()
            mesh.export(os.path.join(FLAGS.out_dir, "mesh_data", "mesh.obj"))
            exit()

...
def shade(
    ...
    alpha = torch.ones_like(kd[..., 0:1])  # kd[..., 3:4] if kd.shape[-1] == 4 else torch.ones_like(kd[..., 0:1])
    kd = kd[..., 0:3]
    ...
    gb_geometric_normal = ru.prepare_shading_normal(gb_pos, view_pos, None, gb_geometric_normal, gb_tangent, gb_geometric_normal, two_sided_shading=True, opengl=True, use_python=use_python)
    ...
    buffers = {
        ...
        'depth': torch.cat((depth, depth, alpha), dim=-1),
        'normal'    : torch.cat((gb_normal, alpha), dim=-1),
        'geo_normal'    : torch.cat((gb_geometric_normal, alpha), dim=-1)
    }
    return buffers
...
def render_layer(
    ...
    buffers['mask'] = mask
    # buffers['normal'] = torch.cat((gb_normal, torch.ones_like(gb_normal[..., 0:1])), dim=-1)
    ################################################################################
    # Prepare output
    ################################################################################
...
def render_mesh(
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), full_res) as peeler:
        ...
        rast, db = peeler.rasterize_next_layer()
        layers_second = [(render_layer(rast, db, mesh, view_pos, lgt, resolution, spp, msaa, bsdf), rast)]

        # rast, db = peeler.rasterize_next_layer()
        # layers_third = [(render_layer(rast, db, mesh, view_pos, lgt, resolution, spp, msaa, bsdf), rast)]
    ...
    for key in layers[0][0].keys():
        ...
        elif key == 'depth' or key == 'mask' or key == 'normal':
    ...
    for key in layers_second[0][0].keys():
        if key == 'shaded':
            accum = composite_buffer(key, layers_second, background, True)
        elif key == 'depth' or key == 'mask' or key == 'normal':
            accum = composite_buffer(key, layers_second, torch.zeros_like(layers[0][0][key]), True)
        else:
            accum = composite_buffer(key, layers_second, torch.zeros_like(layers[0][0][key]), False)

        # Downscale to framebuffer resolution. Use avg pooling
        out_buffers[key + "_second"] = util.avg_pool_nhwc(accum, spp) if spp > 1 else accum

    # for key in layers_third[0][0].keys():
    #     if key == 'shaded':
    #         accum = composite_buffer(key, layers_third, background, True)
    #     elif key == 'depth' or key == 'mask' or key == 'normal':
    #         accum = composite_buffer(key, layers_third, torch.zeros_like(layers[0][0][key]), True)
    #     else:
    #         accum = composite_buffer(key, layers_third, torch.zeros_like(layers[0][0][key]), False)
    #
    #     # Downscale to framebuffer resolution. Use avg pooling
    #     out_buffers[key + "_third"] = util.avg_pool_nhwc(accum, spp) if spp > 1 else accum
    return out_buffers

# ==============================================================================================
#  Custom mesh renderer for our tetrahedral representation.
# ==============================================================================================


def custom_render_mesh(
        ctx,
        mesh,
        mtx_in,
        view_pos,
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), full_res) as peeler:
        for i in range(2):
            ...
            # Interpolate camera projected position
            gb_c_pos, _ = interpolate(v_pos_clip, rast, mesh.t_pos_idx.int())
            # Interpolate colors
            gb_color, _ = interpolate(mesh.v_color[None, ...], rast, mesh.t_pos_idx.int())
            # Interpolate normals
            gb_normal, _ = interpolate(mesh.v_nrm[None, ...], rast, mesh.t_nrm_idx.int())
            ...
            gb_geometric_normal, _ = interpolate(face_normals[None, ...], rast, face_normal_indices.int())

            # Compute tangent space
            # assert mesh.v_nrm is not None and mesh.v_tng is not None
            if mesh.v_tng is not None:
                gb_tangent, _ = interpolate(mesh.v_tng[None, ...], rast, mesh.t_tng_idx.int()) # Interpolate tangents
            else:
                gb_tangent = None

            gb_normal = ru.prepare_shading_normal(gb_pos, view_pos, None, gb_normal, gb_tangent, gb_geometric_normal, two_sided_shading=True, opengl=True)
            gb_geometric_normal = ru.prepare_shading_normal(gb_pos, view_pos, None, gb_geometric_normal, gb_tangent, gb_geometric_normal, two_sided_shading=True, opengl=True)
            ...
            buffers = {
                ...
                'normal': torch.cat((gb_normal, torch.ones_like(gb_color[..., 0:1])), dim=-1),
                'depth': torch.cat((depth, depth, torch.ones_like(depth)), dim=-1),
                'mask': mask,
                'geo_normal': torch.cat((gb_geometric_normal, torch.ones_like(gb_color[..., 0:1])), dim=-1),
            }

            ################################################################################
            # Prepare output
            ################################################################################
            layers[i] = (buffers, rast)
    ...
    for key in layers[0][0].keys():
        if key == 'shaded':
            accum = composite_buffer(key, layers[0], background, True)
        elif key == 'depth' or key == 'mask' or key == 'normal':
            accum = composite_buffer(key, layers[0], torch.zeros_like(layers[0][0][key]), True)
        else:
            accum = composite_buffer(key, layers[0], torch.zeros_like(layers[0][0][key]), False)

        # Downscale to framebuffer resolution. Use avg pooling
        out_buffers[key] = util.avg_pool_nhwc(accum, spp) if spp > 1 else accum


    for key in layers[1][0].keys():
        if key == 'shaded':
            accum = composite_buffer(key, layers[1], background, True)
        elif key == 'depth' or key == 'mask' or key == 'normal':
            accum = composite_buffer(key, layers[1], torch.zeros_like(layers[0][0][key]), True)
        else:
            accum = composite_buffer(key, layers[1], torch.zeros_like(layers[0][0][key]), False)

        # Downscale to framebuffer resolution. Use avg pooling
        out_buffers[key + "_second"] = util.avg_pool_nhwc(accum, spp) if spp > 1 else accum


    # for key in layers[2][0].keys():
    #     if key == 'shaded':
    #         accum = composite_buffer(key, layers[2], background, True)
    #     elif key == 'depth' or key == 'mask' or key == 'normal':
    #         accum = composite_buffer(key, layers[2], torch.zeros_like(layers[0][0][key]), True)
    #     else:
    #         accum = composite_buffer(key, layers[2], torch.zeros_like(layers[0][0][key]), False)
    #
    #     # Downscale to framebuffer resolution. Use avg pooling
    #     out_buffers[key + "_third"] = util.avg_pool_nhwc(accum, spp) if spp > 1 else accum
